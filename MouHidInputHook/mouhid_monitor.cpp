...
typedef struct _HOOK_CALLBACK_CONTEXT
{
	...
	GET_INPUT_PACKET_REPLY inputPacket;
	KSPIN_LOCK SpinLock;
...
--*/
{
	...
	pCallbackContext = (PHOOK_CALLBACK_CONTEXT)ExAllocatePoolZero(
		NonPagedPool,
		sizeof(*pCallbackContext), 'CC1');
	if (!pCallbackContext)
	{
...
VOID
MhmGetInputPacket(
	PGET_INPUT_PACKET_REPLY pInputPacket
)
{
	KIRQL oldIrql;

	// Acquire the spin lock at the correct IRQL
	KeAcquireSpinLock(&g_MhmContext.CallbackContext->SpinLock, &oldIrql);

	pInputPacket->PacketIndex = g_MhmContext.CallbackContext->inputPacket.PacketIndex;
	pInputPacket->LastX = g_MhmContext.CallbackContext->inputPacket.LastX;
	pInputPacket->LastY = g_MhmContext.CallbackContext->inputPacket.LastY;

	// Release the spin lock and restore IRQL
	KeReleaseSpinLock(&g_MhmContext.CallbackContext->SpinLock, oldIrql);
}
...
)
{
		...
		++pInputPacket)
	{
		...
		// Acquire the spin lock to protect inputPacket
		KeAcquireSpinLock(&pCallbackContext->SpinLock, &oldIrql);

		// Perform the assignment safely
		pCallbackContext->inputPacket.PacketIndex = PacketIndex;
		pCallbackContext->inputPacket.LastX = pInputPacket->LastX;
		pCallbackContext->inputPacket.LastY = pInputPacket->LastY;

		// Release the spin lock
		KeReleaseSpinLock(&pCallbackContext->SpinLock, oldIrql);
