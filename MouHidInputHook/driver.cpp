...
)
{
    ...
    PGET_INPUT_PACKET_REPLY pGetInputPacketReply = NULL;
    ...
    switch (pIrpStack->Parameters.DeviceIoControl.IoControlCode)
    {
        ...
        case IOCTL_GET_INPUT_PACKET:
            DBG_PRINT("Processing IOCTL_GET_INPUT_PACKET.");

            if (cbInput)
            {
                ntstatus = STATUS_INVALID_PARAMETER_4;
                DBG_PRINT("IOCTL_GET_INPUT_PACKET STATUS_INVALID_PARAMETER_4");
                goto exit;
            }

            pGetInputPacketReply = (PGET_INPUT_PACKET_REPLY)pSystemBuffer;
            if (!pGetInputPacketReply)
            {
                ntstatus = STATUS_INVALID_PARAMETER_5;
                DBG_PRINT("IOCTL_GET_INPUT_PACKET STATUS_INVALID_PARAMETER_5");
                goto exit;
            }

            if (sizeof(*pGetInputPacketReply) != cbOutput)
            {
                ntstatus = STATUS_INVALID_BUFFER_SIZE;
                DBG_PRINT("IOCTL_GET_INPUT_PACKET STATUS_INVALID_BUFFER_SIZE");
                goto exit;
            }

            MhmGetInputPacket(pGetInputPacketReply);

            Information = sizeof(*pGetInputPacketReply);
