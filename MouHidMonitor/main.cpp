	...
	for (;;)
	{
		...
		for (i = 0; i < nEventsRead; ++i)
		{
			...
			if (VK_CONTROL ==
				InputEvents[i].Event.KeyEvent.wVirtualKeyCode)
			{
				GET_INPUT_PACKET_REPLY inputPacket{ 0 };
				if (!DrvGetInputPacket(&inputPacket)) {
					ERR_PRINT("DrvGetInputPacket() failed");
					continue;
				}
				INF_PRINT("Packet Index=%lu LX=%ld LY=%ld", inputPacket.PacketIndex, inputPacket.LastX, inputPacket.LastY);
			}
		}
